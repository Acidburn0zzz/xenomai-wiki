...
`--ipipe`:: specifies the path of the interrupt pipeline (aka I-pipe)
    ...
    available from the project's https://xenomai.org/downloads/ipipe/[download
...
- If you plan to build from the sources available from the Xenomai GIT
  ...
  https://xenomai.org/downloads/xenomai/[release tarball].
...
- An interrupt pipeline (I-pipe) patch must be available for your
  ...
  Xenomai project https://xenomai.org/downloads/ipipe/[there].
...
If building from a https://xenomai.org/downloads/xenomai/[release
tarball], a set of autoconf-generated file will be readily available
from the extracted source tree, and therefore reconfiguring will not
be required.
