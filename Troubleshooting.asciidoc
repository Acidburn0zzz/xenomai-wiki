...
link:Porting_Xenomai_To_A_New_Arm_Soc#The_general_case[detailed
information].
...
support.  See
link:Installing_Xenomai_3#Installing_the_Cobalt_core[this document]
...
unprivileged user, which fails because invoking Cobalt services
requires +CAP_SYS_NICE+. However, you may allow a specific group of
users to access Cobalt services, by following the instructions on
link:Running_As_A_Regular_User[this page].

=== WARNING: [main] failed to lock memory

A variant of the previous issue, specific to the _Mercury_ core
though: your application needs the CAP_SYS_NICE and CAP_IPC_LOCK
capabilities to be granted access to the system services the Xenomai
libraries need, such as mlockall(2) (see capabilities(7)). Running the
application with root privileges is a way to gain those capabilities.
...
are available from link:Installing_Xenomai_3[this page] for Xenomai
3, and link:Installing_Xenomai_2[this page] for Xenomai 2.
